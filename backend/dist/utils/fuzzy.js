...
exports.getFuzzyScore = exports.isFuzzyMatch = exports.normalizeForFuzzy = exports.getLevenshteinDistance = void 0;
...
const normalizeForFuzzy = (text) => text
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    ...
    .replace(/\s+/g, " ")
    .trim();
exports.normalizeForFuzzy = normalizeForFuzzy;
const isFuzzyMatch = (query, candidateName, candidateKeywords = []) => {
    const normQuery = (0, exports.normalizeForFuzzy)(query);
    const normCandidate = (0, exports.normalizeForFuzzy)(candidateName);
    const normKeywords = candidateKeywords.map(exports.normalizeForFuzzy);
    ...
    for (const qToken of qTokens) {
        ...
        for (const cToken of cTokens) {
            ...
            const dist = (0, exports.getLevenshteinDistance)(qToken, cToken);
            const prefixDist = (0, exports.getLevenshteinDistance)(qToken, cToken.slice(0, qToken.length));
    ...
    if (queryNoSpace.length > 5)
        allowedNoSpaceDist = 2;
    else if (queryNoSpace.length > 3)
        allowedNoSpaceDist = 1;
    if ((0, exports.getLevenshteinDistance)(queryNoSpace, candidateNoSpace) <= allowedNoSpaceDist)
        return true;
    for (const k of normKeywords) {
        const kNoSpace = k.replace(/\s+/g, "");
        if ((0, exports.getLevenshteinDistance)(queryNoSpace, kNoSpace) <= allowedNoSpaceDist)
            return true;
    }
    return allTokensMatch;
};
exports.isFuzzyMatch = isFuzzyMatch;
const getFuzzyScore = (query, candidateName, candidateKeywords = []) => {
    const normQuery = (0, exports.normalizeForFuzzy)(query);
    const normCandidate = (0, exports.normalizeForFuzzy)(candidateName);
    ...
    if (normCandidate.startsWith(normQuery))
        return 80;
    if (candidateKeywords.some(k => (0, exports.normalizeForFuzzy)(k) === normQuery))
        return 90;
    const qTokens = normQuery.split(" ").filter(Boolean);
...
exports.getFuzzyScore = getFuzzyScore;
